...
async def callback_send_order(callback: types.Message | types.CallbackQuery,
    ...
    async with session as _session:
        ...
        try:
            ...
            _order_text = f'Тип обращения: {valid_request_type}\nКомментарий: {comment}\nДата создания обращения: {datetime.now().astimezone(tz=moscow_tz).strftime("%d.%m.%y %H:%M")}(по мск)\n\nСсылка на <a href="http://65.108.242.208/admin/main/orders/{new_order_id}/change/">Django админку</a>'
            _text += _order_text
